...
void PacMan::update(GameWorld& gw)
{
    ...
    // Lock the grid prior to access
    //std::lock_guard<std::mutex> lock(gw.grid_mutex);
    gw.grid_mutex.lock();
    ...
    else if (gw.grid[row][col] == Tile_PowerUp)
    {
        ...
        // Lock the ghost array prior to access
        //std::lock_guard<std::mutex> ghost_lock(gw.ghost_array_mutex);
