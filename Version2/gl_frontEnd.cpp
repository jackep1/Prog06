...
void drawGameWorld(const GameWorld *game_world)
{
    ...
    for (int i = 0; i < game_world->ghost_count; i++)
    {
        ...
        //std::lock_guard<std::mutex> lock(game_world->ghost_array_mutex);  // Lock the ghost array prior to access
    ...
    const GLfloat angle_incr = F_PI_TIMES_2 / static_cast<GLfloat>(tri_count);
...
void makeObjectGeometry(const int numRows, const int numCols)
{
    ...
    GLfloat angle_iter = 0.0f;
    constexpr GLfloat angle_incr = F_PI / 16.0f;
    const GLfloat rw = DH * ghost_scale_factor;
    const GLfloat rh = DV * ghost_scale_factor;
    while (angle_iter <= F_PI + 0.00001f)
    {
        const GLfloat x = cosf(angle_iter) * rw;
        const GLfloat y = sinf(angle_iter) * rh;
        glVertex2f(x, y);
        angle_iter += angle_incr;
    }
    glEnd();
...
//
void myResize(const int w, const int h)
{
...
void gw_closetIntersection(const int x, const int y, int *out_x, int *out_y)
{
    *out_x = *out_y = 0;
    if (gw != nullptr)
    {
